...
void save_s()
{
	...
	<<att_id<<endl<<arm_id<<endl<<item_id<<endl<<arena_lock<<endl<<info<<endl<<difficulty<<endl;
	save.close();
    delay(1000);
...
void load_s()
{
	...
	if (load) {
		...
		ydar>>promah>>proid>>win>>lose>>death>>att_id>>arm_id>>item_id>>arena_lock>>info>>difficulty;
		load.close();
		delay(1000);
	} else {
		system("cls");
